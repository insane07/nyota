...
// return true if set is modified by inserting new value
func (set StringSet) Add(strs ...string) bool {
	modified := false
	for _, str := range strs {
		modified = modified || !set.Contains(str)
		set[str] = struct{}{}
	}
...
// Pop - removes random n elements from the set
func (set StringSet) Pop(n int) {
	if n < 1 {
		return
	}
